---
title: V-Agent and trusted worktrees
description: "Configure which folders V-Agent trusts for running code and extensions."
---
...
To let users choose based on their own threat model and risk tolerance, all worktrees start in Restricted Mode. Restricted Mode prevents downloading and running related items from `.zed/settings.json`. Until a worktree is trusted, V-Agent does not run related untrusted actions and waits for user confirmation. This gives users a chance to review project settings, MCP servers, and language servers.

V-Agent still trusts tools it installs globally. Global MCP servers and global language servers such as Prettier and Copilot are installed and started as usual, independent of worktree trust.

If a worktree is not trusted, V-Agent will indicate this with an exclamation mark icon in the title bar. Clicking this icon or using the `workspace::ToggleWorktreeSecurity` action will bring up the security modal that allows the user to trust the worktree.
...
This command will restart V-Agent, to ensure no untrusted settings, language servers or MCP servers persist.

This feature works locally and on SSH and WSL remote hosts. V-Agent tracks trust information per host in these cases.
...
By default, V-Agent does not trust new worktrees. Users must trust each new worktree individually. Though not recommended, users can trust all worktrees with this setting ([how to edit](./configuring-zed.md#settings-files)):
...
V-Agent has multiple layers of trust, based on the requests, from the least to most trusted level:

- "single file worktree"

After opening an empty V-Agent window, you can open a single file. You can also open a file outside the current directory after opening a directory.
...
Spawning a language server presents a risk should the language server experience a supply-chain attack; therefore, V-Agent restricts that by default. Each single file worktree requires a separate trust grant, unless the directory containing it is trusted or all worktrees are trusted.

- "directory worktree"

If a directory is open in V-Agent, it is a full worktree. It may spawn multiple language servers and MCP servers defined in project settings. Each directory worktree therefore requires a separate trust grant unless a parent-directory trust grant exists (see below).
