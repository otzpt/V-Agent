...
description: "Debugger Extensions for V-Agent extensions."
---
...
V-Agent offers an automatic way to create debug scenarios with _debug locators_.
...
See DAP servers published as extensions [in the extension marketplace](https://zed.dev/extensions?filter=debug-adapters) (V-Agent has no marketplace of its own, so this is upstream Zed's).
