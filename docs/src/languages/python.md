---
title: Python
description: "Configure Python language support in V-Agent, including language servers, formatting, and debugging."
---

# How to Set Up Python in V-Agent

Python support is available natively in V-Agent.
...
You'll need both V-Agent and Python installed before you can begin.

### Step 1: Install Python

V-Agent does not bundle a Python runtime, so you’ll need to install one yourself.
...
## Open Your First Python Project in V-Agent

Once V-Agent and Python are installed, open a folder containing Python code to start working.

### Step 1: Launch V-Agent with a Python Project
...
V-Agent will recognize `.py` files automatically using its native tree-sitter-python parser, with no plugins or manual setup required.

### Step 2: Use the Integrated Terminal (Optional)

V-Agent includes an integrated terminal, accessible from the bottom panel. If V-Agent detects that your project is using a [virtual environment](#virtual-environments), it will be activated automatically in newly-created terminals. You can configure this behavior with the [`detect_venv`](../reference/all-settings.md#terminal-detect_venv) setting.
...
- Language server settings ("workspace configuration"), which must be configured per-editor (using `settings.json` in V-Agent's case) but apply to all projects opened in that editor
...
Language server settings for basedpyright in V-Agent can be set in the `lsp` section of your `settings.json`.
...
### How V-Agent Uses Python Toolchains

V-Agent uses the selected Python toolchain for your project in the following ways:
...
- If the toolchain is a virtual environment, the environment's activation script will be run automatically when you launch a new shell in V-Agent's integrated terminal, giving you convenient access to the selected Python interpreter and dependency set.
- If a built-in language server is installed in the active virtual environment, that binary will be used instead of V-Agent's private automatically-installed binary. This also applies to debugpy.

### Selecting a Toolchain

For most projects, V-Agent will automatically select the right Python toolchain. In complex projects with multiple virtual environments, it might be necessary to override this selection. You can use the [toolchain selector](../toolchains.md#selecting-toolchains) to pick a toolchain from the list discovered by V-Agent, or [specify the path to a toolchain manually](../toolchains.md#adding-toolchains-manually) if it's not on the list.

## Code Formatting & Linting

V-Agent uses [Ruff](https://github.com/astral-sh/ruff) for formatting and linting Python code. Specifically, it runs Ruff as an LSP server using the `ruff server` subcommand.

### Configuring Formatting

Formatting in V-Agent follows a two-phase pipeline: first, code actions on format (`code_actions_on_format`) are executed, followed by the configured formatter.
...
Like basedpyright, Ruff reads options from both V-Agent's language server settings and configuration files (`ruff.toml`) when used in V-Agent. Unlike basedpyright, _all_ options can be configured in either of these locations, so the choice of where to put your Ruff configuration comes down to whether you want it to be shared between projects but specific to V-Agent (in which case you should use language server settings), or specific to one project but common to all Ruff invocations (in which case you should use `ruff.toml`).

Here's an example of using language server settings in V-Agent's `settings.json` to disable all Ruff lints in V-Agent (while still using Ruff as a formatter):
...
V-Agent supports syntax highlighting for code embedded in Python strings by adding a comment with the language name.
...
V-Agent supports Python debugging through the `debugpy` adapter. You can start with no configuration or define custom launch profiles in `.zed/debug.json`.

### Start Debugging with No Setup

V-Agent can automatically detect debuggable Python entry points. Press F4 (or run debugger: start from the Command Palette) to see available options for your current project.
This works for:
...
V-Agent uses `debugpy` under the hood, but no manual adapter configuration is required.

### Define Custom Debug Configurations

For reusable setups, create a `.zed/debug.json` file in your project root. This gives you more control over how V-Agent runs and debugs your code.
...
Issues with Python in V-Agent typically involve virtual environments, language servers, or tooling configuration.
...
- Check your V-Agent log (using the {#action zed::OpenLog} action) for errors related to the language server you're trying to use. This is where you're likely to find useful information if the language server failed to start up at all.
...
- Restart V-Agent to reinitialize language server connections, or try restarting the language server using the {#action editor::RestartLanguageServer}

If the language server is failing to resolve imports, and you're using a virtual environment, make sure that the right environment is chosen in the selector. You can use the "Server Info" view to confirm which virtual environment V-Agent is sending to the language server&mdash;look for the `* Configuration` section at the end.
