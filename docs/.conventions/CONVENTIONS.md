# V-Agent Documentation Conventions
...
- **Version-specific behavior changes** — Include version callouts (e.g., "In V-Agent v0.224.0 and above...")
...
- Users would search for it **by name** (e.g., "V-Agent terminal", "V-Agent snippets")
...
```yaml
---
title: Feature Name - V-Agent
description: One sentence describing what this page covers. Used in search results.
---
```

- `title`: Feature name, optionally with "- V-Agent" suffix for SEO
...
- Commands: `:w`, `:q`
- File paths: `~/.config/v-agent/settings.json`
- Action names: `git::Commit`
- Values: `true`, `false`, `"eager"`

### Action and Keybinding References

Use V-Agent's special syntax for dynamic rendering:
...
- V-Agent has no marketing site of its own; do not add `zed.dev` marketing links
...
Keep language docs focused on V-Agent-specific configuration, not general language tutorials.
...
- **macOS/Linux:** `~/.config/v-agent/settings.json`
