...
pub struct AgentSettingsContent {
    ...
    /// Command to automatically run when V-Agent creates a Terminal Thread shell in the agent panel.
    ...
    /// external agent's permission system is only used when V-Agent would allow
...
pub struct SandboxPermissionsContent {
    ...
    /// object `{requested, resolved}`; V-Agent writes objects (the canonical,
    /// symlink-resolved target established at approval time), while
    /// hand-authored entries may be bare path strings. Paths written by
    /// V-Agent are absolute.
    /// Default: []
