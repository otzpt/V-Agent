...
impl Workspace {
    ...
    pub fn toggle_panel_focus<T: Panel>(
        ...
        // True open/close toggle: if panel T is already the visible (open +
        // active) panel in its dock, close it; otherwise open + focus it. This
        // is focus-independent, so it behaves the same whether triggered by a
        // keybinding, the status bar, or the activity-bar rail — click to open,
        // click again to close.
        let already_visible = self.all_docks().iter().any(|dock| {
            let dock = dock.read(cx);
            dock.is_open()
                && dock.panel_index_for_type::<T>().is_some()
                && dock.panel_index_for_type::<T>() == dock.active_panel_index()
        });

        let did_focus_panel = if already_visible {
            self.close_panel::<T>(window, cx);
            false
        } else {
            self.focus_or_unfocus_panel::<T>(window, cx, &mut |_, _, _| true);
            true
        };
    ...
    /// V-Agent activity bar: a vertical icon rail on the far left that toggles
    /// the main panels (Explorer, Search, Git, AI, Terminal, Extensions, Settings).
    /// Actions are dispatched by registered name so this stays dependency-free.
    fn render_activity_bar(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let colors = cx.theme().colors();
        let bg = colors.title_bar_background;
        let border = colors.border;
        let items: [(IconName, &'static str, &'static str); 7] = [
            (IconName::FileTree, "explorer", "project_panel::ToggleFocus"),
            (IconName::MagnifyingGlass, "search", "pane::DeploySearch"),
            (IconName::GitBranch, "git", "git_panel::ToggleFocus"),
            (IconName::Sparkle, "ai", "assistant::ToggleFocus"),
            (IconName::Terminal, "terminal", "terminal_panel::ToggleFocus"),
            (IconName::Blocks, "extensions", "zed::Extensions"),
            (IconName::Settings, "settings", "zed::OpenSettings"),
        ];
        v_flex()
            .id("vagent-activity-bar")
            .h_full()
            .w_12()
            .flex_none()
            .bg(bg)
            ...
            .pt_2()
            .gap_1()
            .children(items.into_iter().map(|(icon, id, action_name)| {
                        ...
                        div()
                            ...
                            .child(
                                h_flex()
                                    ...
                                    .size_full()
                                    .overflow_hidden()
                                    // V-Agent activity bar rail on the far left.
