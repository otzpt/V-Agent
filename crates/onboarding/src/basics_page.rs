...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = "V-Agent can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.";

    SwitchField::new(
        "onboarding-auto-trust-worktrees",
        Some("Trust All Projects By Default"),
        Some("Automatically mark all new projects as trusted to unlock all V-Agent's features".into()),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
            move |&selection, _, cx| {
...
/// only thing a new user actually needs is Ollama itself.
fn render_local_ai_button(_cx: &mut App) -> impl IntoElement {
    AgentSetupButton::new("local-ai-onboarding")
        .icon(
...
fn render_ai_section(_user_store: &Entity<UserStore>, cx: &mut App) -> impl IntoElement {
    ...
    v_flex()
        ...
        .child(
            Label::new(
                "V-Agent is local-first. Install Ollama and models are detected \
                 automatically — no account, no API key, nothing leaves your machine. \
                 Bring your own key or an external agent instead if you prefer.",
            )
            .color(Color::Muted),
        )
        .child(grid)
