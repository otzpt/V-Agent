...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("V-Agent.exe"), p("old\\V-Agent.exe")),
        Job::mkdir(p("old\\bin")),
        Job::move_file(p("bin\\V-Agent.exe"), p("old\\bin\\V-Agent.exe")),
        ...
        // Copy new files
        Job::move_file(p("install\\V-Agent.exe"), p("V-Agent.exe")),
        Job::move_file(p("install\\bin\\V-Agent.exe"), p("bin\\V-Agent.exe")),
...
fn release_file_handles(app_dir: &Path) -> Result<()> {
    ...
    let files_to_release = [
        app_dir.join("V-Agent.exe"),
        app_dir.join("bin\\V-Agent.exe"),
...
pub(crate) fn perform_update(app_dir: &Path, hwnd: Option<isize>, launch: bool) -> Result<()> {
    ...
    if launch {
        ...
        let _ = std::process::Command::new(app_dir.join("V-Agent.exe")).spawn();
    }
