...
impl TerminalProcessIds {
    ...
    fn process_group_ids(self) -> impl Iterator<Item = i32> {
        std::iter::once(self.child)
            ...
            // V-Agent itself, so never let a zero id (or a negative one from an
...
/// Fetches V-Agent-relevant Pseudo-Terminal (PTY) process information
...
impl PtyProcessInfo {
    ...
    /// Updates the cached process info, emitting a [`Event::TitleChanged`] event if the V-Agent-relevant info has changed
