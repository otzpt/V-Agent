    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        children.push(
            h_flex()
                .h_full()
                .gap_0p5()
                // V-Agent brand mark at the far left of the title bar.
                .child(
                    h_flex().px_1p5().child(
                        Icon::from_path("icons/vagent.svg")
        ...
        children.push(
            h_flex()
                ...
                // No Zed account UI: sign-in, the "Signing in…" indicator and
                // the user menu all authenticate against Zed Industries, which
                // V-Agent has no account system for. Provider logins that do
                // work (GitHub Copilot, Claude Code and other ACP agents) live
                // in the agent panel and are unaffected.
                .into_any_element(),
        );
        ...
        match status {
            ...
            client::Status::UpgradeRequired => {
                ...
                let label = match auto_updater.map(|auto_update| auto_update.read(cx).status()) {
                    Some(AutoUpdateStatus::Updated { .. }) => "Please restart V-Agent to Collaborate",
                    Some(AutoUpdateStatus::Installing { .. })
                    | Some(AutoUpdateStatus::Downloading { .. })
                    | Some(AutoUpdateStatus::Checking) => "Updating...",
                    Some(AutoUpdateStatus::Idle)
                    | Some(AutoUpdateStatus::Errored { .. })
                    | None => "Please update V-Agent to Collaborate",
                };
                        ...
                        this.custom_entry(
                            move |_window, _cx| {
                                h_flex()
                                    ...
                                    .gap_1()
                                    .justify_between()
                                    .child(Label::new("Restart to update V-Agent").color(Color::Accent))
                                    .child(
